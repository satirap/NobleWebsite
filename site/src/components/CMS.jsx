...
import { NobleStore as S } from '../store.js';
...
function CmsImageField({ label, value, onChange, lang }) {
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
        <div style={{ width: 70, height: 50, flex: 'none', borderRadius: 'var(--radius-sm)', background: value ? '#fff' : 'var(--ink-100)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
          {value ? <img src={value} alt="" style={{ width: '100%', height: '100%', objectFit: 'contain' }} /> : <Ico n="image" s={18} />}
        </div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          <button style={btn} onClick={() => setMode(m => m === 'lib' ? null : 'lib')}><Ico n="grid-3x3" s={12} />{t('คลัง', 'Library')}</button>
          <button style={btn} onClick={() => setMode(m => m === 'url' ? null : 'url')}><Ico n="link" s={12} />URL</button>
          <button style={btn} onClick={() => fileRef.current && fileRef.current.click()}><Ico n="upload" s={12} />{t('อัปโหลด', 'Upload')}</button>
          {value && <button style={{ ...btn, color: 'var(--text-muted)' }} onClick={() => onChange('')}><Ico n="x" s={12} />{t('ลบ', 'Clear')}</button>}
...
export default function CMS({ lang, data, onChange }) {
  const [open, setOpen] = useState(false);
  const [tab, setTab] = useState('hero');
  const t = (th, en) => (lang === 'th' ? th : en);
  const edit = (fn) => { const d = S.clone(data); fn(d); onChange(d); };
  const img = (label, value, set) => <CmsImageField label={label} value={value} onChange={set} lang={lang} />;
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px' }}>
          ...
          {tab === 'hero' && (
            <>
              ...
              <CmsField label={t('หัวข้อใหญ่', 'Headline')} value={data.hero.title[lang]} onChange={(v) => edit(d => d.hero.title[lang] = v)} area={2} />
            ...
            <>
              ...
              {data.services.map((s, i) => (
                <CmsCard key={i} onRemove={data.services.length > 1 ? () => edit(d => d.services.splice(i, 1)) : null} removeLabel={t('ลบ', 'Remove')}>
                  ...
                  <CmsField label={t('ไอคอน (Lucide)', 'Icon (Lucide name)')} value={s.icon} onChange={(v) => edit(d => d.services[i].icon = v)} mono />
